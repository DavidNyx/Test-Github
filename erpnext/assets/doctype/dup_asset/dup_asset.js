...
frappe.provide("erpnext.dup_asset");
frappe.provide("erpnext.accounts.dimensions");

frappe.ui.form.on('Dup_Asset', {
	onload: function(frm) {
	...
	setup: function(frm) {
		frm.make_methods = {
			'Dup_Asset_Movement': () => {
				frappe.call({
				method: "erpnext.assets.doctype.dup_asset.dup_asset.make_asset_movement",
				freeze: true,
				args:{
				...
				callback: function (r) {
					if (r.message) {
						...
						console.assert(doc.name != undefined, "Ok m ngu")
					}
				}
			});
			},
	...
	refresh: function(frm) {
		frappe.ui.form.trigger("Dup_Asset", "is_existing_asset");
		...
		if (frm.doc.docstatus==1) {
			if (in_list(["Submitted", "Partially Depreciated", "Fully Depreciated"], frm.doc.status)) {
				frm.add_custom_button(__("Transfer Asset"), function() {
					erpnext.dup_asset.transfer_asset(frm);
				}, __("Manage"));

				frm.add_custom_button(__("Scrap Asset"), function() {
					erpnext.dup_asset.scrap_asset(frm);
				}, __("Manage"));
				...
				frm.add_custom_button(__("Restore Asset"), function() {
					erpnext.dup_asset.restore_asset(frm);
				}, __("Manage"));
			}
	...
	make_journal_entry: function(frm) {
		frappe.call({
			method: "erpnext.assets.doctype.dup_asset.dup_asset.make_journal_entry",
			args: {
	...
	set_finance_book: function(frm) {
		frappe.call({
			method: "erpnext.assets.doctype.dup_asset.dup_asset.get_item_details",
			args: {
	...
	opening_accumulated_depreciation: function(frm) {
		erpnext.dup_asset.set_accumulated_depreciation(frm);
	},
	...
	make_sales_invoice: function(frm) {
		frappe.call({
			args: {
				"dup_asset": frm.doc.name,
				"item_code": frm.doc.item_code,
				"company": frm.doc.company,
				"serial_no": frm.doc.serial_no
			},
			method: "erpnext.assets.doctype.dup_asset.dup_asset.make_sales_invoice",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_maintenance: function(frm) {
		frappe.call({
			...
			method: "erpnext.assets.doctype.dup_asset.dup_asset.create_asset_maintenance",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_repair: function(frm) {
		frappe.call({
			...
			method: "erpnext.assets.doctype.dup_asset.dup_asset.create_asset_repair",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	split_asset: function(frm) {
		...
		dialog.set_primary_action(__('Split'), function() {
			...
			frappe.call({
				...
				method: "erpnext.assets.doctype.dup_asset.dup_asset.split_asset",
				callback: function(r) {
	...
	create_asset_value_adjustment: function(frm) {
		frappe.call({
			...
			method: "erpnext.assets.doctype.dup_asset.dup_asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
...
frappe.ui.form.on('Depreciation Schedule', {
	make_depreciation_entry: function(frm, cdt, cdn) {
		...
		if (!row.journal_entry) {
			frappe.call({
				method: "erpnext.assets.doctype.dup_asset.depreciation.make_depreciation_entry",
				args: {
					"asset_name": frm.doc.name,
...
erpnext.dup_asset.set_accumulated_depreciation = function(frm) {
...
erpnext.dup_asset.scrap_asset = function(frm) {
	frappe.confirm(__("Do you really want to scrap this asset?"), function () {
		frappe.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpnext.assets.doctype.dup_asset.depreciation.scrap_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpnext.dup_asset.restore_asset = function(frm) {
	frappe.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		frappe.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpnext.assets.doctype.dup_asset.depreciation.restore_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpnext.dup_asset.transfer_asset = function() {
	frappe.call({
		method: "erpnext.assets.doctype.dup_asset.dup_asset.make_asset_movement",
		freeze: true,
		args:{
