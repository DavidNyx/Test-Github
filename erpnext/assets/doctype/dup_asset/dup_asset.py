...
from erpnext.assets.doctype.dup_asset.depreciation import (
...
class Dup_Asset(AccountsController):
	def validate(self):
	...
	def on_cancel(self):
		...
		make_reverse_gl_entries(voucher_type="Dup_Asset", voucher_no=self.name)
	...
	def cancel_movement_entries(self):
		...
		for movement in movements:
			movement = frappe.get_doc("Dup_Asset_Movement", movement.get("name"))
			movement.cancel()
...
def update_maintenance_status():
	assets = frappe.get_all("Dup_Asset", filters={"docstatus": 1, "maintenance_required": 1})

	for asset in assets:
		asset = frappe.get_doc("Dup_Asset", asset.name)
...
def make_post_gl_entry():
	...
	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			...
			for asset in assets:
				doc = frappe.get_doc("Dup_Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = frappe.get_meta("Dup_Asset")
...
def create_asset_repair(asset, asset_name):
	...
	asset_repair.update({"dup_asset": asset, "asset_name": asset_name})
	return asset_repair
...
def create_asset_value_adjustment(asset, asset_category, company):
	...
	asset_value_adjustment.update(
		{"dup_asset": asset, "company": company, "asset_category": asset_category}
	)
...
def transfer_asset(args):
	...
	movement_entry = frappe.new_doc("Dup_Asset_Movement")
	...
	frappe.msgprint(
		...
		.format("<a href='/app/Form/Dup_Asset_Movement/{0}'>{0}</a>")
...
def make_journal_entry(asset_name):
	asset = frappe.get_doc("Dup_Asset", asset_name)
	(
		fixed_asset_account,
...
def make_asset_movement(assets, purpose=None):
	...
	asset_movement = frappe.new_doc("Dup_Asset_Movement")
	asset_movement.quantity = len(assets)
	for asset in assets:
		asset = frappe.get_doc("Dup_Asset", asset.get("name"))
		asset_movement.company = asset.get("company")
		asset_movement.append(
			"assets",
			{
				"dup_asset": asset.get("name"),
