...
def get_depreciable_assets(date):
	return frappe.db.sql_list(
		...
		from tabDup_Asset a, `tabDepreciation Schedule` ds
...
def scrap_asset(asset_name):
	...
	for entry in get_gl_entries_on_asset_disposal(asset):
		entry.update({"reference_type": "Dup_Asset", "reference_name": asset_name})
		je.append("accounts", entry)

	je.flags.ignore_permissions = True
	je.submit()

	frappe.db.set_value("Dup_Asset", asset_name, "disposal_date", today())
	frappe.db.set_value("Dup_Asset", asset_name, "journal_entry_for_scrap", je.name)
