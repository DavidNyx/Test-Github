...
class Dup_Asset_Movement(Document):
	def validate(self):
		self.validate_asset()
	...
	def validate_asset(self):
		for d in self.assets:
			status, company = frappe.db.get_value("Dup_Asset", d.asset, ["status", "company"])
		...
		for d in self.assets:
			if self.purpose in ["Transfer", "Issue"]:
				if not d.source_location:
					d.source_location = frappe.db.get_value("Dup_Asset", d.asset, "location")

				if not d.source_location:
					frappe.throw(_("Source Location is required for the Asset {0}").format(d.asset))

				if d.source_location:
					current_location = frappe.db.get_value("Dup_Asset", d.asset, "location")
	...
	def validate_employee(self):
		for d in self.assets:
			if d.from_employee:
				current_custodian = frappe.db.get_value("Dup_Asset", d.asset, "custodian")
	...
	def set_latest_location_in_asset(self):
		...
		for d in self.assets:
			args = {"dup_asset": d.asset, "company": self.company}
			...
			latest_movement_entry = frappe.db.sql(
				...
				FROM `tabDup_Asset_Movement_Item` asm_item, `tabDup_Asset_Movement` asm
				WHERE
					asm_item.parent=asm.name and
					asm_item.asset=%(dup_asset)s and
			...
			frappe.db.set_value("Dup_Asset", d.asset, "location", current_location)
			frappe.db.set_value("Dup_Asset", d.asset, "custodian", current_employee)
